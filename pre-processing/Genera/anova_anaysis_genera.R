# 读取数据
data <- read.table("20%_CSS_cor/otu_taxon_Genus_CSS_INT_mGWAS.txt",
                   header = TRUE,
                   row.names = 1,
                   sep = "\t",
                   check.names = FALSE,
...
setwd("H:/data/性状/菌/Genus")
# 确保分组变量为因子
data$sex <- as.factor(cov$sex)
data$batch <- as.factor(cov$batch)

# 读取要分析的菌属名
traits <- readLines("20%_CSS_cor/p_list_Genus.txt")
...
# 保存为 CSV
write.csv(results, "20%_CSS_cor/genus_CSS_ANOVA_results.csv", row.names = FALSE)
...
# 保存为 CSV
write.csv(results_wide, "20%_CSS_cor/genus_CSS_ANOVA_cov-group.csv", row.names = FALSE)
